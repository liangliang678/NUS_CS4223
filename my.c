...
int finish_my = 0;
pthread_mutex_t mutex_finish_my;
long long max_cycle;

int check_share_my(uint32_t tag, uint32_t index)
{
    int ret = 0;
...
void simulate_my(){
    ...
    pthread_mutex_init(&mutex_finish_my, NULL);
    pthread_barrier_init(&barrier, NULL, 4);
    pthread_t core0, core1, core2, core3;
    int arg_core0 = 0, arg_core1 = 1, arg_core2 = 2, arg_core3 = 3;
    pthread_create(&core0, NULL, my_core, &arg_core0);
    pthread_create(&core1, NULL, my_core, &arg_core1);
    pthread_create(&core2, NULL, my_core, &arg_core2);
    pthread_create(&core3, NULL, my_core, &arg_core3);
...
void* my_core(void* core_num_pointer)
{
    ...
    while(fgets(line, 100, input_file)){
        ...
        while(cycle < next_inst){
            int wb = snoop_bus_my(core_num, state, tag, &cycle);
            if(!wb){
                cycle++;
                pthread_barrier_wait(&barrier);
            }
        ...
        if(line[0] == '0'){
            ...
            if(hit_flag == 0){
                ...
                // wait for bus ack
                int recv = 0;
                while(!(recv = bus_recv_my(core_num))){
                    int wb = snoop_bus_my(core_num, state, tag, &cycle);
                    if(!wb){
                        cycle++;
                        bus_idle++;
                ...
                if(state[refill_way * block_num + addr_index] == M || state[refill_way * block_num + addr_index] == Sm){
                    ...
                    int counter = 100;  
                    while(counter != 0){
                        int wb = snoop_bus_my(core_num, state, tag, NULL);
                        assert(!wb);
                        cycle++;
                        mem_idle++;
                        counter--;
                ...
                state[refill_way * block_num + addr_index] = (check_share_my(addr_tag, addr_index) == 0) ? E : Sc;
        ...
        // store
            ...
            if(hit_flag == 0){
                ...
                // refill
                state[refill_way * block_num + addr_index] = (check_share_my(addr_tag, addr_index) == 0) ? M : Sm; 
                tag[refill_way * block_num + addr_index] = addr_tag;
                lru[refill_way * block_num + addr_index] = 1;
                bus_cancle(core_num);
                ...
                if (check_share_my(addr_tag, addr_index) == 0){
    ...
    printf("> core%d finish_my at cycle %lld\n", core_num, cycle);
    ...
    pthread_mutex_lock(&mutex_finish_my);
    finish_my++;
    max_cycle = cycle;
    pthread_mutex_unlock(&mutex_finish_my);
