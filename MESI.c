...
int finish_MESI = 0;
pthread_mutex_t mutex_finish_MESI;
long long max_cycle;

int check_share_MESI(uint32_t tag, uint32_t index)
{
    int ret = 0;
...
void simulate_MESI()
{
    ...
    pthread_mutex_init(&mutex_finish_MESI, NULL);
...
void* MESI_core(void* core_num_pointer)
{
    ...
    pthread_mutex_lock(&mutex_finish_MESI);
    finish_MESI++;
    max_cycle = cycle;
